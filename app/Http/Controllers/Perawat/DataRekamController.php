...
class DataRekamController extends Controller
{
    ...
    public function index()
    {
        ...
        $dataPasien= $rekam;
        return view('perawat.pemeriksaan', array(
    ...
    public function proses($id)
    {
        try {
            ...
            if($rekam->count() > 0){
                ...
                return view('perawat.detailpemeriksaan', array(
                    'title' => "Dashboard Administrator | MyKlinik v.1.0",
                    'firstMenu' => 'pemeriksaan',
                    'secondMenu' => 'pemeriksaan',
            ...
            return redirect(route('perawat.pemeriksaan'))->with(['delete' => "Belum ada Pasien hari ini!"]);
        } catch (Exception $e) {
            abort(404);
        }
    }
    ...
    public function selesai_periksa(PeriksaRequest $request)
    {
        ...
        DB::beginTransaction();
        try {
            $this->pendaftaranService->save_pemeriksaan_perawat($request);
            DB::commit();
            return redirect(route('perawat.pemeriksaan'))->with(['success' => "Data berhasil disimpan!"]);
        } catch (Exception $e) {
            DB::rollback();
            return redirect(route('perawat.pemeriksaan'))->withErrors(['error' => $e->getMessage()]);
        }
    }
}
